...
caixi_name = {'lucai','chuancai','yuecai','sucai','mincai','zhecai','xiangcai','huicai','dongbeicai','gangtai','hubeicai','hucai','jiangxicai','jingcai','other',...
 ...
 for i = 1:length(metric)
     ...
     data = load(strcat('data/model_result/',metric{i},'_model',num2str(ing_num_mutate),'_',num2str(gama),'_',num2str(pop_copy),'sim.txt'));
     ...
     dlmwrite(strcat('model_',num2str(ing_num_mutate),'_',num2str(gama),'_',num2str(pop_copy),'_',metric{i},'_vs_geodist.txt'),res,' ');
     disp('with outliers..');
     ...
     sat = find(data);
     res = [data(sat),distance(sat)];
     dlmwrite(strcat('model_',num2str(ing_num_mutate),'_',num2str(gama),'_',num2str(pop_copy),'_',metric{i},'_vs_geodist_deleteoutliers.txt'),res,' ');
